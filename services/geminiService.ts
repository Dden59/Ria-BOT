...
export const getRiaResponse = async (userMessage: string, history: Message[]): Promise<string> => {
  ...
  if (!ai) {
    if (!apiKey) {
      ...
      return "Ой, кажется, у меня возникли технические неполадки. 🤨 Скорее всего, мой создатель забыл добавить секретный ключ в переменные окружения проекта на Vercel. Нужно создать переменную с именем `GEMINI_API_KEY` и вставить в неё ключ. Без него я не могу связаться со своим 'мозгом'. Пожалуйста, проверь это в настройках: Project > Settings > Environment Variables.";
    }
  ...
  try {
    ...
    if (error instanceof Error && (error.message.includes('API key not valid') || error.message.includes('permission') || error.message.includes('API key is invalid'))) {
       return "Хм, тот ключ, что ты добавил в `GEMINI_API_KEY`, кажется неверным или у него нет нужных прав (возможно, не привязан биллинг в Google Cloud?). Проверь его еще раз, пожалуйста. Он должен быть правильным, чтобы я могла работать. 🤨";
    }
